// chapter 16 exercise 6 - ch16e6.cpp
#include <iostream>;
...
int main()
{
	...
	for (int cycle = 0; cycle < cyclelimit; ++cycle)
	{
		...
		{
			if (theList.size() == theList.max_size())
				++turnaways;
			else
			{
				++customers;
		...
		if (wait_time <= 0 && !(line.empty()))
		{
			...
			line.pop();						// pop next customer from the queue
	...
	// add customers left in queue to the
	// number of turnaways
	turnaways += line.size();
